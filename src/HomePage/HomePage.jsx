...
const HomePage = () => {
  ...
  const openChat = (e) => {
    ...
    getChatHistory(loggedInUser, userCLicked).then((result) =>
  ...
  const sendMessage = async (message) => {
    ...
    if (connection) {
      let response = await connection.invoke(
        'SendChatMessage',
        message,
        loggedInUser,
        currentChatUser
      ...
      setChatHistory((chatHistory) => [
        ...chatHistory,
        {
          from: response.from.username,
          to: response.to.username,
          messageText: response.messageText,
          sendTime: response.sendTime.toString('HH:mm'),
        },
      ])
    }
  }
      ...
      <div className={classes.body}>
        <Drawer variant="permanent">
          <div className={classes.toolbarIcon}>
            <IconButton className={classes.contactsIcon}>Contacts</IconButton>
          </div>
          <Divider />
